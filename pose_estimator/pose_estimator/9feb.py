...
class PoseEstimationNode(Node):
    ...
    def scan_callback(self, scan_msg):
        ...
        self.ranges= np.nan_to_num(self.ranges, nan=10.0)
        laser = np.array(self.ranges)
        laser = np.nan_to_num(laser, nan=0.0)
        laser[laser <= 0.02] = 1.0
        self.point_at_min_dist= min(laser)
                
        self.left_dist = np.min(self.ranges[500:512])
        self.front_dist = np.min(self.ranges[246:256])
        self.right_dist = np.min(self.ranges[0:15])
